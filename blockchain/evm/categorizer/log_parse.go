...
func RunLogParse() {
	url := "inproc://spaghetti_evm_log_parses"

	internal_sock, err := zmq.NewSocket(zmq.REP)
	if err != nil {
		log.Fatalf("trying to create evm log parser socket for network id %s: %v", url, err)
	}

	if err := internal_sock.Bind(url); err != nil {
		log.Fatalf("trying to create evm log parser %s: %v", url, err)
	}
	...
	for {
		...
		if err != nil {
			if _, err := internal_sock.SendMessage(err.Error()); err != nil {
				mu.Unlock()
				log.Fatalf("reply.ToString error to send message by %s error: %s", url, err.Error())
			}
		} else {
			if _, err := internal_sock.SendMessage(reply_string); err != nil {
				mu.Unlock()
				log.Fatalf("failed to reply by %s: %s", url, err.Error())
			}
		}
		mu.Unlock()
	}

...
func (suite *TestAbiDbSuite) TestAbi() {
	...
	abi_id := "not_exist"
	exist := ExistInDatabase(suite.db_con, abi_id)
	suite.Require().False(exist)

	// but the one we inserted should be
	exist = ExistInDatabase(suite.db_con, abi.Id)
	suite.Require().True(exist)

	// Select abi from database
	// should fail as abi doesn't exist
	_, err = GetFromDatabaseByAbiId(suite.db_con, abi_id)
	suite.Require().Error(err)

	// select abi that exists
	db_abi, err := GetFromDatabaseByAbiId(suite.db_con, abi.Id)
	suite.Require().NoError(err)

	suite.Require().EqualValues(abi, *db_abi)
}
